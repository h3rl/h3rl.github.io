...
window.onload = function () {
  ...
  function update() {
    ctx.clearRect(0, 0, canvas.width, canvas.height); // Draw particles
    ...
    for (var i = 0; i < particles.length; i++) {
      ...
      for (var j = particles.length - 1; j > i; j--) {
        var distance = Math.sqrt(Math.pow(particles[i].x - particles[j].x, 2) + Math.pow(particles[i].y - particles[j].y, 2));
  ...
  window.addEventListener('resize', function () {
    ...
    canvas.width = pnDivSize.width = pnDiv.offsetWidth;
    canvas.height = pnDivSize.height = pnDiv.offsetHeight; // Reset particles
  ...
  if (options.interactive) {
    ...
    particles.push(mouseParticle); // Mouse event listeners
